...
def read_recommendations(user_id: int, db: Session = Depends(get_db)):
    db_user = model.get_user(db, user_id=user_id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    return crud.get_recommendations_for_user(db, user_id=user_id)

@app.get("/recommendations/", response_model=list[models.Recomendacion])
def read_all_recommendations(db: Session = Depends(get_db)):
    recommendations = crud.get_all_recommendations(db)
...
def read_recommendation(recommendation_id: int, db: Session = Depends(get_db)):
    db_recommendation = crud.get_recommendation(db, recommendation_id=recommendation_id)
