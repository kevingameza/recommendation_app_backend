...
def generar_recomendaciones( user_id, item_id):
    ...
    recomendaciones = modelo.predict(user_id, item_id)  # Ajusta esto según tu modelo
...
@app.get("/recomendaciones/{user_id}", response_model=List[models.RespuestaDeRecomendaciones])
def obtener_recomendaciones(user_id: int, db: Session = Depends(get_db)):
    user_id='user_000370'
    item_id= 'Bad Ischl'
    datos_usuario = {'user_000407': 'Crying In The Shadows'}  # Adapta esto a tus necesidades
