...
def obtener_recomendaciones(
    ...
    # Convert details to a JSON string before saving to the database
    ...
    db_recomendation = models.RecomendacionDB(
        ...
        details=details_str  # Save the JSON string instead of the dict
    )

    db.add(db_recomendation)
    db.commit()

    # Convert back to dict before returning (if necessary for the response model)
    return models.Recomendacion(
        ...
        details=json.loads(db_recomendation.details)  # Convert from JSON string to dict
    )
