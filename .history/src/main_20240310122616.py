...
def get_recommendation_by_id(recommendation_id: int, db: Session = Depends(get_db)):
    ...
    details_str = json.dumps(recommendation.details)
    if not recommendation:
        raise HTTPException(status_code=404, detail="Recommendation not found")
    return models.Recomendacion(
    uid=recommendation.uid,
    iid=recommendation.iid,
    r_ui=recommendation.r_ui,
    est=recommendation.est,
    details=details_str  # Use the proper dictionary format here
)
