...
def get_all_recommendations(db: Session = Depends(get_db)):
    ...
    return models.Recomendacion(
    uid=recommendations.uid,
    iid=recommendations.iid,
    r_ui=recommendations.r_ui,
    est=recommendations.est,
    details=json.loads(recommendations.details)  # Asegúrate de que esto es un diccionario
)
