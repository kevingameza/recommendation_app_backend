...
@app.get("/recomendaciones/{user_id}", response_model=models.Recomendacion)
def obtener_recomendaciones(user_id: str = Path(..., description="The ID of the user"),
                            item_id: str = Query(None, description="The ID of the item"),
                            rating: float = Query(None, description="The rating given by the user")):
    # Make sure to handle the cases where item_id or rating could be None
    if item_id is None or rating is None:
        raise HTTPException(status_code=400, detail="Item ID and rating must be provided")

    recomendaciones = generar_recomendaciones(user_id, item_id, rating)
